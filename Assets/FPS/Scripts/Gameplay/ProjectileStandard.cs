...
namespace Unity.FPS.Gameplay
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        void Update()
        {
            ...
            // Hit detection
            {
                ...
                if (foundHit)
                {
                    ...
                    Instantiate(explosionPoint, explosionPoint.transform.position, explosionPoint.transform.rotation);
                }
            }
        ...
        void PushObject(Collider collision)
        {
            Collider[] objects = Physics.OverlapSphere(transform.position, 1f);
            foreach (Collider part in objects)
            {
                Rigidbody rb = part.GetComponent<Rigidbody>();
                if (collision.gameObject.name == "Cube")
                {
                    rb.AddExplosionForce(700f, transform.position, 1f);
                }
            }
        }
